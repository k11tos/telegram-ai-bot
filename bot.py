...
async def load_gateway_presets(app) -> tuple[bool, bool]:
...
async def reload_presets_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    loaded_from_gateway, used_fallback = await load_gateway_presets(context.application)
    presets = get_presets_from_bot_data(context.application.bot_data)

    if loaded_from_gateway and not used_fallback:
        await update.message.reply_text(
            f"프리셋을 다시 불러왔습니다: {', '.join(presets.keys())}"
        )
        return
